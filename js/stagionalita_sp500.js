...
async function loadData() {
    try {
        // Carica i dati dell'sp500
        const sp500Response = await fetch('csv/sp500_montly_historical.csv');
        ...
        const years = [...new Set(sp500Data.map(d => d.date.getFullYear()))].sort((a, b) => a - b);
        ...
        // Event listeners
    startYearInput.addEventListener('change', () => updateChart(sp500Data, monthlyRates));
    endYearInput.addEventListener('change', () => updateChart(sp500Data, monthlyRates));
        document.getElementById('convertToEur').addEventListener('change', () => updateChart(sp500Data, monthlyRates));
        modeSelect.addEventListener('change', () => updateChart(sp500Data, monthlyRates));

        // Crea il grafico iniziale
        updateChart(sp500Data, monthlyRates);
    } catch (error) {
...
function calculateSeasonality(sp500Data, monthlyRates, startYear, endYear, convertToEur, mode) {
...
function updateChart(sp500Data, eurUsdRates) {
    ...
    const monthlyStats = calculateSeasonality(sp500Data, eurUsdRates, startYear, endYear, convertToEur, mode);
    ...
    let label, yTitle;
    if (mode === 'absolute') {
        label = `Prezzo medio sp500 (${currency}/oncia)`;
    ...
    seasonalityChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: {
                    display: true,
                    text: `Stagionalità sp500 ${startYear}-${endYear}`,
                    font: {
                        size: 16
                    }
                },
